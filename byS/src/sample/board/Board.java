...
public class Board {
    ...
    public  void getShapes() {
        ...
        Integer id= 0;
        Field firstField = new Field(id++, 0 , 0, Colours.GREEN);
...
//        Dolny masyw gwiazdy
        for(int i = 0; i < size + 1; i++){
            ...
            for(int j = 0; j <= 2 * size + i; j++){
                ...
                if(j < i)
                    newField.setColour(Colours.YELLOW);
                if(j > 2 * size)
                    newField.setColour(Colours.VIOLET);
                if(j < 2 *  size + i)
                newField.setUpRight(oldLine.get(j));
                if(j > 0){
                    newField.setUpLeft(oldLine.get(j - 1));
        ...
        //najwyzszy rzad dolnego ramienia gwiazdy
        oldLine = newLine;
        newLine = new ArrayList<>();
        for(int i = 0; i <  size; i++){
            Field newField = new Field(id++, 2 * i - (size - 1) ,2 * (3 * size + 1) );

            newField.setColour(Colours.ORANGE);
        ...
        for(int i = 1; i < size; i++){
            ...
            for(int j = 0; j < size - i; j++){
                Field newField = new Field(id++, 2 * j - (size - 1 - i), 2 * (3 * size + 1 +  i));
                newField.setColour(Colours.ORANGE);
                newField.setUpLeft(oldLine.get(j));
                newField.setUpRight(oldLine.get(j + 1));
                if(j > 0)
